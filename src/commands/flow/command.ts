    ...
    programTag.addCommand(
        new Command()
            ...
            .action(async (branch: string, commandOptions: any) => {
                ...
                const flowManager = new FlowManager()
                const existingTags = await flowManager.listBranchTags(branch, options.channel ?? null)
    ...
    programTag.addCommand(
        new Command()
            ...
            .action(async (branch: string, commandOptions: any) => {
                const options = { ...program.opts(), ...commandOptions }
                const flowManager = new FlowManager()
                const latestBranchTag = await flowManager.latestTagVersion(branch, options.channel ?? null)
